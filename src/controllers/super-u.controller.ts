...
export const loginSuperUsuario = async (req: Request, res: Response) => {
	try {
		...
		const isPasswordValid = await compare(password, superUsuario.password);
		...
		const expiration = 60 * 60 * 24 * 1000; // 1 dia
		const token = await generateToken(superUsuario.id, expiration);
